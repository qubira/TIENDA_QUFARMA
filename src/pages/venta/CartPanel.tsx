  ...
  return (
    <aside className="flex w-96 shrink-0 flex-col border-l border-slate-200 bg-white">
      <div className="flex items-center gap-2 border-b border-slate-100 px-4 py-3.5">
        ...
        <span className="ml-auto rounded-full bg-brand-50 px-2 py-0.5 text-xs font-semibold text-brand-700">
          {items.length}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2">
        {items.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-center text-slate-300">
            ...
            <p className="text-sm text-slate-400">Agrega productos para iniciar la venta</p>
          </div>
        ) : (
          <ul className="space-y-2">
            {items.map(({ producto, cantidad }) => {
              ...
              return (
                <li
                  key={producto.id}
                  className="animate-fade-in flex items-start gap-2 rounded-lg border border-slate-100 p-2.5"
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-slate-800">
                      {producto.nombreComercial}
                    </p>
                    <p className="text-xs text-slate-400">{formatCurrency(producto.precioVenta)} c/u</p>
                    <div className="mt-1.5 flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        className="h-6 w-12 rounded-md border border-slate-200 text-center text-xs"
                      />
                      <button
      ...
      <div className="space-y-1.5 border-t border-slate-100 px-4 py-3 text-sm">
        ...
        <div className="flex justify-between pt-1 text-base font-bold text-slate-800">
          <span>Total</span>
